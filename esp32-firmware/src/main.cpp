 ...
 * 
 * v1.0.7 变更：
 *   - 极简固件：仅 Serial + 系统信息 + 心跳，不初始化任何外设
 *   - 目的：验证 PlatformIO + espressif32@6.5.0 平台层能否正常启动
 *   - 修正 PSRAM 配置：memory_type = dio_opi（Flash DIO + PSRAM OPI）
 *   - 添加 -DBOARD_HAS_PSRAM 启用 PSRAM 支持
 *   - USB 模式改为 ARDUINO_USB_MODE=1（硬件 USB，避免 TinyUSB 问题）
 *   - 删除 TinyUSB 库依赖
 * 
 * 预期行为：
...
void setup()
{
    ...
    Serial.println();
    Serial.println("========================================================");
    Serial.println("  QingYun ESP32-S3-CAM Firmware v1.0.7");
    Serial.println("  MINIMAL TEST - Platform Validation");
    Serial.println("  DIO + OPI PSRAM + HW USB");
    Serial.println("========================================================");
    Serial.println();
...
void loop()
{
    ...
    Serial.printf("[v1.0.7] Heartbeat #%d | Heap: %u | PSRAM: %u\n",
                  counter,
                  ESP.getFreeHeap(),
                  ESP.getFreePsram());
