    ...
    private fun showFloatingWindow() {
        ...
        wv.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                view?.evaluateJavascript("if(typeof onVisionResult==='function'){console.log('[V2.9.118] ✅策略引擎就绪')}else{console.log('[V2.9.118] ❌策略引擎未加载！')}", null)
            }
        }
            ...
            try {
                ...
                if (result != null) {
                    ...
                    handler.post {
                        ...
                        executeJs("(function(){try{if(typeof onVisionResult==='function'){onVisionResult($resultJson);if(typeof AndroidBridge!=='undefined'&&AndroidBridge.confirmVisionReceived){AndroidBridge.confirmVisionReceived()}}else{console.log('[V2.9.118] onVisionResult不存在,尝试重载HTML');if(typeof AndroidBridge!=='undefined'&&AndroidBridge.showAdvice){AndroidBridge.showAdvice('COLOR:FOLD|SIGNAL:ERROR|REASON:策略引擎未加载');}setTimeout(function(){location.reload();},1000);}}catch(e){console.log('[V2.9.118] onVisionResult异常:'+e.message);if(typeof AndroidBridge!=='undefined'&&AndroidBridge.showAdvice){AndroidBridge.showAdvice('COLOR:FOLD|SIGNAL:ERROR|REASON:JS异常:'+e.message.substring(0,30));}}})()")
                        tvAction?.alpha = 1.0f
